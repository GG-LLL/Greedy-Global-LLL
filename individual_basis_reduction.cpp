/*******************************************************************************
 * Program:     individual_basis_reduction
*******************************************************************************/
#include "common.h"
...
int main (int argc, char * argv[]) {
	...
	if (argc != 5) {
		...
		cout << "6: " << _BKZ_08_ << endl;
		cout << "7: " << _BKZ_10_ << endl;
		cout << "8: " << _BKZ_12_ << endl;
		cout << "9: " << _BKZ_20_ << endl;
		exit(-1);
	}
	...
	switch (algo) {
		...
		case 5:
			...
			break;
		case 6:
			sprintf (dir, "%s/%s",_PARENT_OUTPUT_DIR_, _BKZ_08_);
			break;
		case 7:
			sprintf (dir, "%s/%s",_PARENT_OUTPUT_DIR_, _BKZ_10_);
			break;
		case 8:
			sprintf (dir, "%s/%s",_PARENT_OUTPUT_DIR_, _BKZ_12_);
			break;
		case 9:
			sprintf (dir, "%s/%s",_PARENT_OUTPUT_DIR_, _BKZ_20_);
			break;
		default:
	...
	for (int i=basis_number_start; i<=basis_number_end; i++) {
		...
		switch (algo) {
			...
			case 2: // _SS_LLL_
      				begin_time = clock();
      				ppOriginalBasis = NTL_SS_LLL (ppOriginalBasis, m, n, delta, &nSwapCount, &nReductionCount);
      				end_time = clock();
				break;
			case 3: // _SS_GGLLL_
      				begin_time = clock();
			...
			case 5: // _Pot_GGLLL_
      				begin_time = clock();
      				ppOriginalBasis = NTL_PotGGLLL (ppOriginalBasis, m, n, eta, &nSwapCount, &nReductionCount);
      				end_time = clock();
				break;
			case 6: // _BKZ_08_
      				begin_time = clock();
      				ppOriginalBasis = BKZ_NTL (ppOriginalBasis, m, n, delta_threshold, 8, 0);
      				end_time = clock();
				break;
			case 7: // _BKZ_10_
      				begin_time = clock();
      				ppOriginalBasis = BKZ_NTL (ppOriginalBasis, m, n, delta_threshold, 10, 0);
      				end_time = clock();
				break;
			case 8: // _BKZ_12_
      				begin_time = clock();
      				ppOriginalBasis = BKZ_NTL (ppOriginalBasis, m, n, delta_threshold, 12, 0);
      				end_time = clock();
				break;
			case 9: // _BKZ_20_
      				begin_time = clock();
      				ppOriginalBasis = BKZ_NTL (ppOriginalBasis, m, n, delta_threshold, 20, 0);
      				end_time = clock();
				break;
			default:
				cout << "No such basis reduction algorithm." << endl;
	...
	cout << nSwapCount << "\t";
 	cout << nReductionCount << "\t";
   	cout << time_taken << "\t";
   	cout << V1NormOutput << "\t";
   	cout << SVPOutputFactor << "\t";
        cout << RHFOutput << "\t";
   	cout << ODOutput << "\t";
   	cout << PotOutput << "\t";
   	cout << SSOutput << "\n";
	...
	long double AverageSwaps = (long double)(totalSwaps)/(long double)(nIterations);
   	long double AverageReductions = (long double)(totalReductions)/(long double)(nIterations);
   	long double AverageTime = (long double)(totalTime)/(long double)(nIterations);
   	RR AverageV1Norm; AverageV1Norm = totalV1Norm / RRIterations;
   	RR AverageSVPFactor; AverageSVPFactor = totalSVPFactor / RRIterations;
    	RR AverageRHF; AverageRHF = (totalRHF)/RRIterations;
   	RR AverageOD; AverageOD = (totalOD)/RRIterations;
   	RR AveragePot; AveragePot = (totalPot)/RRIterations;
   	RR AverageSS; AverageSS = (totalSS)/RRIterations;

	cout << AverageSwaps << "\t";
 	cout << AverageReductions << "\t";
   	cout << AverageTime << "\t";
   	cout << AverageV1Norm << "\t";
   	cout << AverageSVPFactor << "\t";
        cout << AverageRHF << "\t";
   	cout << AverageOD << "\t";
   	cout << AveragePot << "\t";
   	cout << AverageSS << "\n";
